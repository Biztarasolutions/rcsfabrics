...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  const handleWishlist = (e: React.MouseEvent) => {
    ...
    else { addWish({ id: `w-${product.id}`, productId: product.id, product, createdAt: new Date().toISOString() }); toast.success('Added to wishlist!'); }
  };
  ...
  return (
      ...
      <div onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
        ...
        <div className="relative overflow-hidden bg-gray-50 dark:bg-dark-700" style={{ aspectRatio: '4/5' }}>
          ...
          <button onClick={handleWishlist} className={`absolute right-3 top-3 flex h-8 w-8 items-center justify-center rounded-full border transition-all duration-200 ${isWishlisted ? 'border-red-200 bg-red-50 text-red-500 dark:border-red-900 dark:bg-red-950' : 'border-white/60 bg-white/80 text-gray-500 opacity-0 backdrop-blur-sm group-hover:opacity-100 dark:border-dark-600 dark:bg-dark-700/80 dark:text-gray-300'}`} aria-label="Wishlist">
            <svg className="h-4 w-4" fill={isWishlisted ? 'currentColor' : 'none'} stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"/></svg>
          </button>
          <div className={`absolute bottom-0 left-0 right-0 p-3 transition-all duration-300 ${hovered ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`}>
            <button onClick={handleAddToCart} className="w-full rounded-xl bg-white/90 px-4 py-2.5 text-sm font-semibold text-gray-900 shadow-md backdrop-blur-sm transition-colors hover:bg-primary-600 hover:text-white dark:bg-dark-800/90 dark:text-white">Quick Add — 0.5m</button>
          </div>
        </div>
        {/* Content */}
          ...
          {product.colors && product.colors.length > 0 ? (
            <div className="flex gap-1 mt-0.5">
              {product.colors.map((c, idx) => {
                const bgColor = (() => {
                  if (c.hexCode) {
                    return c.hexCode.startsWith('#') ? c.hexCode : `#${c.hexCode}`;
                  }
                  return c.name || '#CCC';
                })();
                return (
            ...
            <div
              className="h-4 w-4 rounded-full border border-gray-300"
              style={{ backgroundColor: product.color.startsWith('#') ? product.color : `#${product.color}` }}
              title={product.color}
            />
          ) : null
          <div className="mt-auto pt-3">
            <div className="flex items-center gap-1.5">
              <div className="flex">{[...Array(5)].map((_, i) => (<svg key={i} className={`h-3.5 w-3.5 ${i < Math.round(product.rating) ? 'stars-filled' : 'stars-empty'}`} fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/></svg>))}</div>
